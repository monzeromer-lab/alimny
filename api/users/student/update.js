...
updateRouter.post("/" , IsAuthenteacted , IsOwner , (req , res , next)=>{
    MySqlDatabase.query(`SELECT * FROM students WHERE email = ${MySqlDatabase.escape(req.UserData.data.email)};` , (err , result)=>{
        err ? next(err) : result.length == 0 ? res.status(401).json({error : true , message : "please signup" , data: []}) : 
    MySqlDatabase.query(`UPDATE students SET username = ${MySqlDatabase.escape(req.body.username)} , birth_date = ${MySqlDatabase.escape(req.body.birth_date)} , first_name = ${MySqlDatabase.escape(req.body.first_name)}, last_name = ${MySqlDatabase.escape(req.body.last_name)} WHERE email = ${MySqlDatabase.escape(req.UserData.data.email)};` , (err , result)=>{
